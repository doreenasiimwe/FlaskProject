...
def all_food_items():
    ...
    return jsonify({"success":True,"data":food_items,"total":len(food_items)}),200
...
def new_food_item():
    ...
    new_food_item = FoodItem(name=name,created_by=created_by,created_at=datetime.now()) 
      
    #inserting values
    db.session.add(new_food_item)
    db.session.commit()
    return jsonify({'message':'New food FoodItem created sucessfully','data': new_food_item}),201
